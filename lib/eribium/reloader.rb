module Eribium
  module Reloader
    ...
    # @param [Eribium::Application] eribium_app The current Active Admin app
    # @param [String] rails_version The version of Rails we're using. 
    #
    # @returns A concrete subclass of AbstractReloader
    def self.build(rails_app, eribium_app, rails_version)
      reloader_class = rails_version[0..2] == "3.2" ? Rails32Reloader : RailsLessThan31Reloader
      reloader_class.new(rails_app, eribium_app, rails_version)
    end

    class AbstractReloader

      attr_reader :eribium_app, :rails_app, :rails_version

      def initialize(rails_app, eribium_app, rails_version)
        @rails_app = rails_app
        @eribium_app = eribium_app
    ...
    # files have actually changed. Eribium had built this functionality
    ...
    class Rails32Reloader < AbstractReloader
      ...
      def attach!
        eribium_app.load_paths.each do |path|
    ...
    class RailsLessThan31Reloader < AbstractReloader
      ...
      def watched_paths
        paths = eribium_app.load_paths
        eribium_app.load_paths.each{|path| paths += Dir[File.join(path, "**", "*.rb")]}
        paths
      end
